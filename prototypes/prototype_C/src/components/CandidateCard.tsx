...
const CandidateCard: React.FC<CandidateCardProps> = ({ color, hasMic = false, showBubble = false, bubbleText, isTyping = false, bubbleLabel = "Introduction", isSpeaking = false, isPaused = false, reactionEmoji }) => {
  ...
  return (
    ...
    >
      ...
      <div className={`candidate-robot ${hasMic ? "has-mic" : ""}`}>
          {reactionEmoji && (
            <div className={`avatar-reaction avatar-reaction-${color}`}>
              {reactionEmoji}
            </div>
          )}
          <video
